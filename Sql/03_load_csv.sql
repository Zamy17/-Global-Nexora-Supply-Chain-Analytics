...
SET @rows_before = (SELECT COUNT(*) FROM stg_supply_chain);

-- Uncomment only when the staging table must be completely reloaded.
...
SET
    ...
    order_date = CASE WHEN NULLIF(TRIM(@order_date), '') IS NULL THEN NULL ELSE STR_TO_DATE(TRIM(@order_date), '%Y-%m-%d %H:%i:%s') END,
    ...
    shipping_date = CASE WHEN NULLIF(TRIM(@shipping_date), '') IS NULL THEN NULL ELSE STR_TO_DATE(TRIM(@shipping_date), '%Y-%m-%d %H:%i:%s') END,
    ...
    customer_first_order_date = CASE WHEN NULLIF(TRIM(@customer_first_order_date), '') IS NULL THEN NULL ELSE STR_TO_DATE(TRIM(@customer_first_order_date), '%Y-%m-%d %H:%i:%s') END,
    customer_last_order_date = CASE WHEN NULLIF(TRIM(@customer_last_order_date), '') IS NULL THEN NULL ELSE STR_TO_DATE(TRIM(@customer_last_order_date), '%Y-%m-%d %H:%i:%s') END,
...
SET @load_status = CASE
    WHEN @rows_inserted >= @expected_min_rows THEN 'SUCCESS'
...
SET process_status = @load_status,
    ...
    completed_at = NOW(6),
    error_message = CASE
        WHEN @rows_inserted >= @expected_min_rows THEN NULL
        WHEN @rows_inserted > 0 THEN CONCAT(
            'Only ', FORMAT(@rows_inserted, 0),
            ' rows were loaded; expected at least ', FORMAT(@expected_min_rows, 0), '.'
        )
...
SELECT
    ...
    CASE
        WHEN @rows_inserted >= @expected_min_rows THEN 'PASS'
